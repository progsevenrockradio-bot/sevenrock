...
Alpine.data('adminConfirmDialog', () => ({
    open: false,
    ...
    submitter: null,
    init() {
        window.addEventListener('lucille:confirm-open', (event) => {
            this.openFromEvent(event);
        });
    },
    openFromEvent(event) {
        const detail = event?.detail ?? {};
        this.form = detail.form instanceof HTMLFormElement ? detail.form : null;
        this.submitter = detail.submitter instanceof HTMLElement ? detail.submitter : null;
        this.title = detail.title ?? 'Confirmar acción';
        this.message = detail.message ?? '¿Deseas continuar?';
...
document.addEventListener('submit', (event) => {
    ...
    window.dispatchEvent(new CustomEvent('lucille:confirm-open', {
        detail: {
            form,
            submitter: event.submitter ?? null,
            title: form.dataset.confirmTitle?.trim() || 'Confirmar acción',
            message,
            confirmLabel: form.dataset.confirmAction?.trim() || 'Confirmar',
            cancelLabel: form.dataset.confirmCancel?.trim() || 'Cancelar',
            tone: form.dataset.confirmTone?.trim() || 'danger',
