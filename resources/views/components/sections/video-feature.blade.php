...
@if ($video)
    @php $backgroundImage = $image ?? $video->image_url; @endphp
    ...
    <section class="relative bg-cover bg-center bg-no-repeat lg:bg-fixed" style="background-image: url('{{ $backgroundImage }}');">
        ...
        <div class="relative mx-auto max-w-[898px] px-6 py-[90px] text-center lg:px-8">
            <x-ui.section-heading :title="$ui['featured_video']" :subtitle="$video->title" />

            <a href="{{ $video->youtube_url }}" target="_blank" rel="noreferrer" class="lucille-video-card group mt-[60px] block overflow-hidden">
