<x-layouts.admin :title="'Radio Artists - '.$themeSettings->site_name">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="mt-2 text-[#9a9a9a]">Local editorial profile cache used by the player modal and drawer.</p>
        </div>
        <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
            <form action="{{ route('admin.radio-artists.index') }}" method="GET" class="flex">
                ...
                @if(request('search'))
                    <a href="{{ route('admin.radio-artists.index') }}" class="ml-2 text-xs text-[#9a9a9a] hover:text-white self-center transition-colors">Limpiar</a>
                @endif
            </form>
    ...
    <div class="overflow-hidden border border-[#2b2b2b] bg-[rgba(16,16,18,.88)]">
        <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-[#2b2b2b] text-[#9a9a9a]">
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-5 py-10 text-center text-[#9a9a9a]">No radio artists yet.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
