<x-layouts.admin :title="'Band Profiles - '.$themeSettings->site_name">
    @if (session('status'))
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="font-display text-3xl uppercase tracking-[.12em] text-[#dcdcdc]">Band Profiles</h1>
            <p class="mt-2 text-[#7b7b7b]">Local editorial profile cache used by the player modal and drawer.</p>
        </div>
        <a href="{{ route('admin.band-profiles.create') }}" class="lucille-button-solid">New Band Profile</a>
    </div>
    ...
    <div class="overflow-hidden border border-[#2b2b2b] bg-[rgba(16,16,18,.88)]">
        <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-[#2b2b2b] text-[#7b7b7b]">
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-5 py-10 text-center text-[#7b7b7b]">No band profiles yet.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
