...
<div class="min-h-screen flex flex-col" style="background: linear-gradient(180deg, #1a0000 0%, #0e0000 30%, #121212 100%);">
    ...
    <div class="px-4 pb-4">
        ...
        {{-- Carga el widget de RadioBoss para tracks recientes --}}
        <script>
        (function() {
            // Widget ID de RadioBoss para tracks recientes
            const STATION_ID = '{{ $stationId }}';
            const WIDGET_ID  = '378';

            // Cuando el DOM esté listo, cargamos los tracks del endpoint JSON de RadioBoss
            async function loadRecentTracks() {
                try {
                    const res  = await fetch(`https://c30.radioboss.fm/w/api.php?u=${STATION_ID}&mode=recent&limit=5`);
                    const data = await res.json();
                    const tracks = data.history || data.recent || [];

                    const container = document.getElementById('recent-tracks-container');
                    if (!container || !tracks.length) return;

                    container.innerHTML = tracks.slice(0, 5).map((t, i) => `
                        <div class="flex items-center gap-3 px-4 py-3 ${i < tracks.length - 1 ? 'border-b border-[#1e1e1e]' : ''} bg-[#1a1a1a]">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm text-white font-semibold truncate">${t.title || t.song || 'Canción'}</p>
                                <p class="text-xs text-gray-500 truncate">${t.artist || ''}</p>
                            </div>
                            <span class="text-[10px] text-gray-600">${t.time || t.date || ''}</span>
                        </div>
                    `).join('');
                } catch {
                    // Silencioso — los skeletons permanecen
                }
            }

            loadRecentTracks();
            // Actualizar cada 30 segundos
            setInterval(loadRecentTracks, 30000);
        })();
        </script>
    </div>

    {{-- Widget oficial de RadioBoss (oculto, para compatibilidad) --}}
    <div style="display:none;">
        <img id="rbcloud_np_live_cover" src="{{ $artworkUrl }}" alt="">
        <script src="https://c30.radioboss.fm/w/nowplaying.js?u={{ $stationId }}&wid=pwa_live&nl=1" defer></script>
    </div>
